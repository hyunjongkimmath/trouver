...
# %% auto 0
__all__ = ['DEFAULT_NUMBERED_ENVIRONMENTS', 'DEFAULT_ENVIRONMENTS_TO_NOT_DIVIDE_ALONG', 'remove_comments', 'divide_preamble',
...
def _create_notes_from_parts(
    ...
    title_numbering_folder_map = {
        title: (numbering, folder) for numbering, (title, folder)
        in headings_folder_correspondence.items()}
    current_section, current_subsection = None, None # section/subsection titles
    ...
    for part in parts:
        current_section, current_subsection = _create_part_or_update(
            parts, title_numbering_folder_map, vault, reference_folder,
...
def _create_part_or_update(
    ...
    if _part_is_of_section(part):
        ...
        _, folder = title_numbering_folder_map[current_section]
    ...
    created_note = _create_note_for_part(
        ...
        reference_name, template_mf, current_section)
...
def _create_note_for_part(
    ...
    mf.add_line_in_section(
        'Topic', {'type': MarkdownLineEnum.DEFAULT, 'line': note_contents})
    # TODO: verify that the below line is sufficient as opposed to adding a space
    mf.lines[-1]['line'] += note_title
    _, section_folder = title_numbering_folder_map[current_section]
    rel_path = (
        reference_folder / section_folder/ f"{reference_name}_{note_title}.md")
...
def _update_links_to_make(
    ...
    note_title = part[0]
    links_to_make[current_section][current_subsection_key].append(
        f'- [[{vn.name}]], {note_title}'
    )
