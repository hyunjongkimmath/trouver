# AUTOGENERATED! DO NOT EDIT! File to edit: ../../nbs/08_machine_learning_55.llm_typo_finding.ipynb.

# %% auto 0
__all__ = ['FIND_LATEX_TYPOS_SYSTEM_PROPT', 'LaTeXCorrection', 'TypoReport', 'audit_note_content', 'log_audit_to_staging',
           'run_vault_audit', 'batch_proofread_notes']

# %% ../../nbs/08_machine_learning_55.llm_typo_finding.ipynb 2
...
# %% ../../nbs/08_machine_learning_55.llm_typo_finding.ipynb 4
...
# %% ../../nbs/08_machine_learning_55.llm_typo_finding.ipynb 5
...
# %% ../../nbs/08_machine_learning_55.llm_typo_finding.ipynb 7
...
#     """
#     Finds typos in a note. 
#     Returns a tuple of (TypoReport or None, Raw_String_Response).
#     """
#     note_content = note.text()
#     user_msg = f"TARGET TEXT TO AUDIT:\n{note_content}"
#     if context_text:
...
#     run_config = {"temperature": 0.0, **(config or {})}
    
#     if verbose: print(f"  → Requesting audit for [[{note.name}]]...")
...
#         # Auto-fix unescaped LaTeX backslashes for JSON compatibility
#         clean = re.sub(r'\\(?![\\"/bfnrtu])', r'\\\\', clean)
        
#         report = TypoReport.model_validate_json(clean)
#         return report, raw_response
#     except Exception as e:
#         if verbose: print(f"  ⚠️ Validation failed. Saving raw output.")
#         return None, raw_response


# %% ../../nbs/08_machine_learning_55.llm_typo_finding.ipynb 8
def audit_note_content(
    model_obj: SupportedLLM,
    note: VaultNote,
    context_text: Optional[str] = None,
    ...
    if use_processed_note_content:
        note_content = process_standard_information_note(
...
# %% ../../nbs/08_machine_learning_55.llm_typo_finding.ipynb 9
...
# %% ../../nbs/08_machine_learning_55.llm_typo_finding.ipynb 10
def run_vault_audit(
...
# %% ../../nbs/08_machine_learning_55.llm_typo_finding.ipynb 28
def batch_proofread_notes(
    ...
    # glossary = extract_notation_from_notes(notes, model_obj, config=config, verbose=verbose)
    
    # for i, note in enumerate(notes):
    #     # Fresh read for the resume check
    #     current_content = staging_note.text()
    #     if resume and f"## [[{note.name}]]" in current_content:
    #         if verbose: print(f"[{i+1}/{len(notes)}] ⏭️ Skipping [[{note.name}]]")
    #         continue
            
    #     if verbose: print(f"[{i+1}/{len(notes)}] 🔍 Auditing [[{note.name}]]...")
        
    #     note_errors = []
    #     chunks = split_note_into_chunks(note.text(), max_chars=chunk_size)
        
    #     for j, chunk in enumerate(chunks):
    #         if verbose and len(chunks) > 1:
    #             print(f"    ∟ Chunk {j+1}/{len(chunks)}...")
            
    #         # Pass 1 & 2 logic is inside run_math_proofreader_with_retry
    #         report = run_math_proofreader_with_retry(
    #             model_obj, chunk, 
    #             f"{FIND_LATEX_TYPOS_SYSTEM_PROPT}\n\nContext:\n{glossary}",
    #             config=config, verbose=verbose
    #         )
            
    #         if report and report.errors:
    #             note_errors.extend([e for e in report.errors if is_fix_safe(e.original, e.correction)])

    #     # Immediate Disk Sync
    #     append_to_staging(note, staging_note, note_errors, verbose=verbose)
